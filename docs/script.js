/*
福岡ピックルボール試合情報 - JavaScript（公開版）
v1.0: ten-shi（テニス）版を移植
       - localStorage キーを pick_ に変更（テニス版と干渉しない）
       - スクロールトップボタンの色をオレンジに変更

機能:
- 絞り込みフィルター（区分 × 曜日 × 主催 の掛け合わせ AND）
  - 区分 / 曜日: 単一選択（全て/個別）
  - 主催: 複数選択（選択した主催の OR）
- お気に入り（♥）: localStorage 保存・カード右上ハート
- 私好み（利用者カスタムプリセット）: 最大5件
- フィルタ結果0件の日付ヘッダー/月見出しを自動で非表示
- アコーディオン開閉
- 状態保存（localStorage）

データ属性（generate_html.py が各カードに付与）:
  data-category = カテゴリ値（一般/シニア/ジュニア/オープン）
...
(function () {
    ...
    // ===== localStorage キー（pick_ 接頭辞でテニス版と分離） =====
    const LS = {
        cat: 'pick_filter_cat',
        day: 'pick_filter_day',
        gen: 'pick_filter_gen',
        orgs: 'pick_filter_orgs',
        accordion: 'pick_accordion',
        orgAcc: 'pick_org_accordion',
        favs: 'pick_favorites',
        presets: 'pick_my_presets'
    };
    ...
    window.saveCurrentPreset = function () {
        ...
        const name = prompt('この絞り込みに名前をつけて保存します');
    ...
    function initScrollTop() {
        ...
        btn.style.cssText = 'position:fixed;bottom:30px;right:30px;width:50px;height:50px;border-radius:50%;background:#E65100;color:white;border:none;font-size:1.5rem;cursor:pointer;display:none;box-shadow:0 4px 8px rgba(0,0,0,0.2);z-index:1000;';
    ...
    document.addEventListener('DOMContentLoaded', () => {
        ...
        applyFilters();
        initScrollTop();
        console.log('✅ 福岡ピックルボール試合情報 - フィルター起動完了 (v1)');
    });
})();
